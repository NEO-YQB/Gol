...
export function ProductCarouselSection({ block }: { block: { id: string; data: Record<string, unknown>; products?: ProductSummary[] } }) {
  ...
  return (
    <section className="mb-8" key={block.id}>
      ...
      <div className="flex items-stretch justify-center gap-5 overflow-x-auto pb-2">
        {products.map((product) => (
          <ProductCard className="h-full min-h-[100%] min-w-[280px]" key={product.id} product={product} />
        ))}
      </div>
    </section>
  )
