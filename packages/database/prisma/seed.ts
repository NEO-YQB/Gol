...
async function clearDemoData() {
  console.log('Clearing existing demo/business data...');
  ...
  await prisma.promotionOnProducts.deleteMany({});
  await prisma.promotionOnStores.deleteMany({});
  await prisma.promotionOnCategories.deleteMany({});
  await prisma.platformPromotion.deleteMany({});
  await prisma.couponOnProducts.deleteMany({});
  await prisma.couponOnStores.deleteMany({});
  await prisma.couponOnCategories.deleteMany({});
  await prisma.coupon.deleteMany({});
  await prisma.productComposition.deleteMany({});
  await prisma.product.deleteMany({});
  await prisma.productElement.deleteMany({});
  ...
  await prisma.user.deleteMany({
    where: {
      phoneNumber: {
        in: demoPhones.filter((phone) => phone !== '09120000000'),
      },
    },
  });
}
...
async function seedAddresses(users: Awaited<ReturnType<typeof ensureUsers>>) {
  ...
  await prisma.otpCode.createMany({
    data: [
      { phoneNumber: users.customerOne.phoneNumber, code: '111111', expiresAt: daysFromNow(1) },
      { phoneNumber: users.customerTwo.phoneNumber, code: '222222', expiresAt: daysFromNow(1) },
      { phoneNumber: users.vendorOne.phoneNumber, code: '333333', expiresAt: daysFromNow(1) },
    ],
  });
}
...
async function seedCatalogAndStores(users: Awaited<ReturnType<typeof ensureUsers>>) {
  ...
  const bouquetType = await prisma.productType.create({
    data: {
      name: 'دسته گل',
      slug: 'bouquet',
  ...
  const plantType = await prisma.productType.create({
    data: {
      name: 'گیاه آپارتمانی',
      slug: 'indoor-plant',
  ...
  const occasionCategory = await prisma.category.create({
    data: {
      name: 'مناسبتی',
      slug: 'occasion',
  ...
  const plantCategory = await prisma.category.create({
    data: {
      name: 'گیاهان سبز',
      slug: 'green-plants',
  ...
  const rose = await prisma.productElement.create({
    data: { name: 'رز قرمز', type: ElementType.FLOWER, unit: 'شاخه', image: productImage },
  });
  const lily = await prisma.productElement.create({
    data: { name: 'لیلیوم سفید', type: ElementType.FLOWER, unit: 'شاخه', image: productImage },
  });
  const eucalyptus = await prisma.productElement.create({
    data: { name: 'اکالیپتوس', type: ElementType.FILLER, unit: 'شاخه', image: productImage },
  });
  const ceramicPot = await prisma.productElement.create({
    data: { name: 'گلدان سرامیکی', type: ElementType.ACCESSORY, unit: 'عدد', image: productImage },
  });

  const storeOne = await prisma.store.create({
    data: {
      name: 'فلوریا',
      slug: 'floria',
  ...
  const storeTwo = await prisma.store.create({
    data: {
      name: 'سبزکده',
      slug: 'sabzkadeh',
  ...
  const productOne = await prisma.product.create({
    data: {
      ...
      shortDescription: 'رز قرمز با تزیین اکالیپتوس',
      quantity: 20,
      price: 950000,
      ...
      metaTitle: 'دسته گل رز کلاسیک',
      metaDescription: 'ارسال سریع دسته گل رز کلاسیک در تهران',
    },
  });

  const productTwo = await prisma.product.create({
    data: {
      name: 'باکس لیلیوم لوکس',
      slug: 'luxury-lily-box',
      description: 'باکس گل لوکس با لیلیوم سفید و گلدان تزئینی',
      shortDescription: 'باکس گل اقتصادی و شیک',
      quantity: 12,
      price: 1250000,
      mainImage: productImage,
      images: [productImage] as unknown as Prisma.InputJsonValue,
  ...
  const productThree = await prisma.product.create({
    data: {
      name: 'سانسوریا ابلق',
      slug: 'snake-plant-variegated',
      description: 'گیاه سانسوریا مقاوم با گلدان سرامیکی',
      shortDescription: 'گیاه آپارتمانی مقاوم',
      quantity: 15,
      price: 780000,
  ...
  const productFour = await prisma.product.create({
    data: {
      name: 'زاموفیلیا سبز',
      slug: 'zz-plant-green',
      description: 'گیاه زاموفیلیا مناسب محیط‌های اداری',
      shortDescription: 'زاموفیلیا با ماندگاری بالا',
      quantity: 8,
      price: 990000,
      mainImage: productImage,
      images: [productImage] as unknown as Prisma.InputJsonValue,
  ...
  await prisma.productComposition.createMany({
    data: [
      { productId: productOne.id, elementId: rose.id, quantity: 15, elementType: ElementType.FLOWER },
      { productId: productOne.id, elementId: eucalyptus.id, quantity: 5, elementType: ElementType.FILLER },
      { productId: productTwo.id, elementId: lily.id, quantity: 8, elementType: ElementType.FLOWER },
      { productId: productTwo.id, elementId: ceramicPot.id, quantity: 1, elementType: ElementType.ACCESSORY },
      { productId: productThree.id, elementId: ceramicPot.id, quantity: 1, elementType: ElementType.ACCESSORY },
      { productId: productFour.id, elementId: ceramicPot.id, quantity: 1, elementType: ElementType.ACCESSORY },
    ],
  });
...
async function seedPricingAndPolicies(
  ...
  console.log('Creating finance rules, discounts, promotions, and coupons...');

  const globalRule = await prisma.commissionRule.create({
    data: {
      scope: CommissionRuleScope.GLOBAL,
      title: 'قانون عمومی کمیسیون',
      description: 'کمیسیون پیش فرض کل سیستم',
      commissionRate: dec(12),
      systemServiceFeeRate: dec(1.5),
      ...
      reason: 'قانون پایه برای همه فروشگاه‌ها',
    },
  });

  const storeSpecificRule = await prisma.commissionRule.create({
    data: {
      scope: CommissionRuleScope.STORE,
      storeId: stores.storeTwo.id,
      title: 'قانون فروشگاه سبزکده',
      description: 'قانون خاص برای فروشگاه گیاه',
      commissionRate: dec(10),
      systemServiceFeeRate: dec(1),
      ...
      priority: 10,
      createdByUserId: adminId,
      reason: 'همکاری بلندمدت با فروشگاه',
    },
  });

  await prisma.vendorDiscount.createMany({
    data: [
      {
...
async function seedCarts(
  ...
  const cartOne = await prisma.cart.create({
    data: {
      userId: users.customerOne.id,
      items: {
        create: [
  ...
  const cartTwo = await prisma.cart.create({
    data: {
      userId: users.customerTwo.id,
      items: {
        create: [
  ...
  const cartThree = await prisma.cart.create({
    data: {
      userId: users.customerThree.id,
      items: {
        create: [
...
async function seedOrdersAndOperations(input: {
  ...
  const orderOne = await prisma.order.create({
    data: {
      ...
      pricingSnapshot: {
        pricing: { totalAmount: 890000, subtotalBaseAmount: 950000 },
      } as Prisma.InputJsonValue,
      financialSnapshot: {
        appliedRuleId: input.commissionRuleId,
        vendorShareAmount: 768200,
      } as Prisma.InputJsonValue,
  ...
  const orderTwo = await prisma.order.create({
    data: {
      ...
      pricingSnapshot: {
        pricing: { totalAmount: 1150000, subtotalBaseAmount: 1250000 },
      } as Prisma.InputJsonValue,
      financialSnapshot: {
        appliedRuleId: input.commissionRuleId,
        vendorShareAmount: 1030000,
      } as Prisma.InputJsonValue,
  ...
  const orderThree = await prisma.order.create({
    data: {
      ...
      couponDiscountAmount: dec(10000),
      discountAmount: dec(60000),
      commissionRuleId: input.commissionRuleId,
      ...
      settlementAutoReleaseEnabled: false,
      pricingSnapshot: {
        pricing: { totalAmount: 720000, subtotalBaseAmount: 780000 },
      } as Prisma.InputJsonValue,
      financialSnapshot: {
        appliedRuleId: input.commissionRuleId,
        vendorShareAmount: 647000,
      } as Prisma.InputJsonValue,
      ...
      statusHistories: {
        create: [
          ...
          { fromStatus: OrderStatus.PENDING, toStatus: OrderStatus.CANCELLED_BY_CUSTOMER, actorType: OrderActorType.CUSTOMER, actorUserId: input.users.customerThree.id, reason: 'تغییر نظر مشتری', note: 'سفارش لغو شد' },
        ],
      },
    },
  });
    ...
    data: [
      {
        ...
        productId: input.products.productOne.id,
        rating: 5,
        comment: 'کیفیت گل عالی بود و ارسال به موقع انجام شد.',
      },
      {
        userId: input.users.customerTwo.id,
        productId: input.products.productTwo.id,
  ...
  const ticket = await prisma.supportTicket.create({
    data: {
      ...
      title: 'خرابی بخشی از گل‌ها',
      description: 'بعضی از گل‌ها در زمان تحویل آسیب دیده بودند.',
      customerEvidence: [{ type: 'image', url: productImage }] as unknown as Prisma.InputJsonValue,
      settlementBlockedAt: hoursAgo(18),
      escalatedAt: hoursAgo(16),
      financeOutcome: SupportTicketFinanceOutcome.PARTIAL_REFUND,
      financeAmount: dec(150000),
      financeNote: 'برای جبران بخشی از مبلغ برگشت داده شود',
    },
  });

  await prisma.supportTicketNote.createMany({
    data: [
      {
        ticketId: ticket.id,
        actorType: SupportTicketActorType.CUSTOMER,
        actorUserId: input.users.customerOne.id,
        message: 'گل‌ها تازگی لازم را نداشتند.',
        isInternal: false,
      },
      {
        ticketId: ticket.id,
        ...
        message: 'پرونده برای بررسی مالی ارسال شد.',
        isInternal: true,
      },
      {
        ticketId: ticket.id,
        ...
        actorUserId: input.users.admin.id,
        message: 'بازگشت وجه جزئی تایید شد.',
        isInternal: true,
      },
    ],
  });

  const walletOne = await prisma.storeWallet.create({
    data: {
      storeId: input.stores.storeOne.id,
      currentBalance: dec(1798200),
      availableBalance: dec(1030000),
      heldBalance: dec(768200),
    },
  });

  const walletTwo = await prisma.storeWallet.create({
    data: {
      storeId: input.stores.storeTwo.id,
    ...
    data: [
      {
        ...
        amount: dec(768200),
        title: `درآمد معلق سفارش #${orderOne.id}`,
        description: 'سهم فروشنده بعد از تحویل سفارش در held balance ثبت شد',
      ...
      },
      {
        ...
        direction: WalletTransactionDirection.CREDIT,
        amount: dec(1030000),
        title: `آزادسازی درآمد سفارش #${orderTwo.id}`,
        description: 'درآمد سفارش به available balance منتقل شد',
  ...
  await prisma.domainEvent.createMany({
    data: [
      ...
      },
      {
        ...
        payload: { gatewayKey: 'mock-default' },
        status: DomainEventStatus.PROCESSED,
      ...
      },
      {
        ...
        payload: { reason: SupportTicketReason.DAMAGED_FLOWERS },
        status: DomainEventStatus.PENDING,
      },
      {
        ...
        payload: { outcome: SupportTicketFinanceOutcome.PARTIAL_REFUND, amount: 150000 },
        status: DomainEventStatus.PROCESSED,
      ...
      },
      {
        ...
        payload: { amount: 647000, direction: WalletTransactionDirection.CREDIT },
        status: DomainEventStatus.PROCESSED,
